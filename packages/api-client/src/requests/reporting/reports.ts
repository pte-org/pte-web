...
export const REPORT_ENDPOINTS = {
  ...
  publishSession: (sessionPublicId: string) => `/api/v1/reporting/sessions/${sessionPublicId}/publish`,
} as const;
...
export function getAttemptReport(client: ApiClient, attemptPublicId: string): Promise<ReportResponse> {
...
export function preflightReportPublication(
  ...
  return client.request<ReportPublicationReadinessResponse>(REPORT_ENDPOINTS.publicationPreflight(sessionPublicId), {
    method: "POST",
  });
}

export async function publishSessionReports(client: ApiClient, sessionPublicId: string): Promise<void> {
...
export function getReportPublicationSummary(
  ...
  return client.request<ReportPublicationSummaryResponse | null>(REPORT_ENDPOINTS.publicationSummary(sessionPublicId));
}
