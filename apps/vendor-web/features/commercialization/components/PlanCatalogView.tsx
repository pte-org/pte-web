  ...
  return (
    <div className="flex flex-col gap-5">
        ...
        >
          <form className="grid gap-4 md:grid-cols-2" onSubmit={(event) => void save(event)}>
            ...
            <Select
              ...
              value={formType}
              onChange={(event) => { const nextType = event.target.value as PlanType; setFormType(nextType); setForm({ ...form, type: nextType }); }}
            />
      ...
      <CommercialPanel
        ...
        actions={
          <Select id="catalog-type" aria-label="Filter plans by type" options={[{ label: "All", value: "ALL" }, { label: "Exam packages", value: "EXAM_PACKAGE" }, { label: "Capacity add-ons", value: "STUDENT_CAPACITY" }]} value={catalogFilter} onChange={(event) => setCatalogFilter(event.target.value as CatalogFilter)} />
        }
      >
        <DataTable
          columns={[
