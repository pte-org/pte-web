...
import { Alert, Button, Input, PageHeader, ProgressBar, StatCard, UsersIcon } from "@pte/ui";
...
export const QuotaView = (): ReactElement => {
  ...
  return (
    <div className="flex flex-col gap-5">
      ...
      {quota && <>
        <div className="grid gap-4 sm:grid-cols-3"><StatCard label="Students in tenant" value={String(quota.current)} footnote={`of ${quota.limit} total capacity`} icon={<UsersIcon />} progress={usedPercentage} accent="blue" /><StatCard label="Remaining capacity" value={String(quota.remaining)} footnote="students available" accent="mint" /><StatCard label="Last preview" value={preview.data ? String(preview.data.adding) : "—"} footnote="students to add" accent="sky" /></div>
