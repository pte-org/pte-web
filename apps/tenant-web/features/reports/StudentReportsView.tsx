...
export const StudentReportsView = (): ReactElement => {
  ...
  return (
    <div className="flex flex-col gap-5">
      ...
      {reports.error && <Alert tone="error">Unable to load your published results. Please try again.</Alert>}
      {reports.data?.length === 0 && (
        <div className="rounded-lg border border-gray-200 bg-white p-8 text-center">
          <h2 className="font-semibold text-gray-900">No published results yet</h2>
          <p className="mt-2 text-sm text-gray-600">Your results will appear here after your Host publishes them.</p>
        </div>
      )}
      <div className="grid gap-4">
        {reports.data?.map((report) => (
          <article key={report.attemptPublicId} className="rounded-lg border border-gray-200 bg-white p-5">
            ...
            <div className="mt-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-5">
              {report.overall && <ScoreCard label="Overall" score={report.overall.score} sufficient={report.overall.sufficientData} />}
              {report.communicativeSkills.map((skill) => (
                <ScoreCard key={skill.skill} label={skill.skill} score={skill.score} sufficient={skill.sufficientData} />
              ))}
            </div>
          </article>
        ))}
...
function ScoreCard({ label, score, sufficient }: { label: string; score: number | null; sufficient: boolean }): ReactElement {
  return (
